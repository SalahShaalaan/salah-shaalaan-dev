...
export const products = [
  {
    ...
    content: (
      <div>
        <p> The Coffee Market Application is a full-featured MERN (MongoDB, Express.js, React.js, Node.js) web application designed to deliver a seamless and enjoyable online shopping experience for coffee enthusiasts. With an intuitive interface and modern design, users can easily browse a wide range of premium coffee products, view detailed product descriptions, add items to their cart, and securely complete purchases. </p>
        <p className="mt-6"> The platform also includes a powerful admin dashboard that allows administrators to manage all aspects of the store efficiently. From adding and updating coffee products to tracking and fulfilling customer orders, the dashboard provides full control over operations. It also includes user management functionality, enabling admins to view, update, or remove users as needed. Additionally, real-time notifications alert the admin whenever a new order is placed, ensuring timely processing and improved customer satisfaction. </p> <p className="mt-6"> Built with scalability and performance in mind, The Coffee Market Application uses a robust backend powered by Node.js and Express, while MongoDB handles data storage for products, users, and orders. On the front end, React provides a fast, responsive, and interactive experience for users. Whether you're a coffee shop owner looking to expand online or a coffee lover searching for your next favorite blend, this application delivers a professional and reliable solution. </p>
      </div>
    ),
  },
  {
